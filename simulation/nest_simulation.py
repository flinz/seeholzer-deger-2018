...
import json
from meanfield.error_functions import MeanfieldHandler
...
from tools.fitting import fit_gauss_curve
...
gen_params_baserate = {
    ...
    mpr.W_J: 1., #bifurcation parameter
...
class nest_simulator:
    ...
    def __init__(self, handler, record=False, cores=2):
        ...
        if record:
            pass
            #self.fn = "scan_bump_"+str(int(time.time()))+".hdf5"
            #self.record_file = h5py.File( "aha", 'w')
    ...
    def run(self, do_only_setup=False):
        ...
        print "Using network: %s (E: %s, I: %s)" % (self.handler.network,self.handler.network.e.neuron_model.name,self.handler.network.i.neuron_model.name)
        ...
        nest.OneToOneConnect(ex_noise*n_e, nest.GetNodes(ex_cells)[0], params = {'weight': 1., 'receptor_type': ex_receptors['AMPA']}, model= 'static_synapse')
        nest.OneToOneConnect(in_noise*n_i, nest.GetNodes(in_cells)[0], params = {'weight': 1., 'receptor_type': in_receptors['AMPA']}, model= 'static_synapse')
        ...
        if self.params["gen"]["sig_len"] > 0:
            ...
            print "Creating poisson signal from t=%i to t=%i" % (self.params["gen"]["sig_start"],self.params["gen"]["sig_start"]+self.params["gen"]["sig_len"])
            ...
            print "Signal (width %g) will be sent to %i neurons centered at %g" % (self.params["gen"]["sig_width"], len(sig_range), self.params["gen"]["sig_center"])
            ...
            if self.params["gen"]["sig_fade"]:
                ...
                ex_signal = nest.Create('poisson_generator', 1, params={'rate': self.params["gen"]["sig_rate"],'start':start,'stop':start+self.params["gen"]["sig_len"]/2.})
                nest.OneToOneConnect(ex_signal*sig_len, [ex_neurons[i] for i in sig_range], params = {'weight': self.params["gen"]["sig_weight"], 'receptor_type': ex_receptors['AMPA']}, model= 'static_synapse')
                ex_signal = nest.Create('poisson_generator', 1, params={'rate': 0.5*self.params["gen"]["sig_rate"],'start':start+self.params["gen"]["sig_len"]/2.,'stop':stop})
                nest.OneToOneConnect(ex_signal*sig_len, [ex_neurons[i] for i in sig_range], params = {'weight': self.params["gen"]["sig_weight"], 'receptor_type': ex_receptors['AMPA']}, model= 'static_synapse')
            else:
                print "Using non-fading poisson signal."
                ex_signal = nest.Create('poisson_generator', 1, params={'rate': self.params["gen"]["sig_rate"],'start':start,'stop':start+self.params["gen"]["sig_len"]})
                nest.OneToOneConnect(ex_signal*sig_len, [ex_neurons[i] for i in sig_range], params = {'weight': self.params["gen"]["sig_weight"], 'receptor_type': ex_receptors['AMPA']}, model= 'static_synapse')

        # STP CONNECTIONS
        if self.handler.network.i.nmda_synapse.is_stp:
            print "Using E->I stp (%s) with params: U=%f tau_r=%f rau_f=%f" % (tsodyks_model, self.handler.network.i.nmda_synapse.U,self.handler.network.i.nmda_synapse.tau_r,self.handler.network.i.nmda_synapse.tau_f)
        ...
        nest.SetKernelStatus({'rng_seeds' : range(self.params["gen"]["base_seed"]+N_vp+1, self.params["gen"]["base_seed"]+2*N_vp+1)})
        ...
        if self.params["gen"][mpr.EL_NOISE]>0. or not self.params["gen"][mpr.EL_NOISE_ARRAY]==None:
            ...
            if not self.params["gen"][mpr.EL_NOISE_ARRAY] == None:
                ...
                print "Setting variable leak currents with given noise array, mean: %.2f, std: %.2f" % (np.mean(leak_variance),np.std(leak_variance))
                ...
                print "Setting variable leak currents: mean %.1f, std %.1f (seed=%i)" % (np.mean(leak_variance), np.std(leak_variance),self.params["gen"][mpr.EL_SEED])
        ...
        if self.params["gen"]["return_w_mat"]:
            ...
            if self.handler.network.e.nmda_synapse.is_stp:
                print "Using E->E stp (%s) with params: U=%f tau_r=%f rau_f=%f" % (tsodyks_model, self.handler.network.e.nmda_synapse.U,self.handler.network.e.nmda_synapse.tau_r,self.handler.network.e.nmda_synapse.tau_f)
        ...
        if self.params["gen"][mpr.W_NOISE]>0.:
            print "Using w noise with level (multiplied after by w1:%f): %f" % (self.params["gen"][mpr.W_NOISE],self.handler.p_e[npr.W_1])

        print "Using w parameters - w_j: %.2f --> w_0: %.2f, w_1: %.2f, w_sigma: %.2f" % (self.handler.p_mf[mpr.W_J], self.handler.p_e[npr.W_0], self.handler.p_e[npr.W_1], self.handler.p_e[npr.W_SIGMA])
        conndict_EE = { 
            "connection_type": "divergent",
            "synapse_model": "NMDA_EE",
            "mask": {"grid": {"rows": 1, "columns": n_e }},
            "weights": {"gaussian_noisy": {"c": self.handler.p_e[npr.W_0], "p_center": self.handler.p_e[npr.W_1], "sigma": self.handler.p_e[npr.W_SIGMA], "sigma_noise": self.params["gen"][mpr.W_NOISE]*self.handler.p_e[npr.W_1]}},
        ...
        if self.params["gen"]["show_weights"]:
            ...
            tmp = np.zeros(n_e)
            for i in range(n_e):
                 tmp[i]= self.handler.p_e[npr.W_0]+self.handler.p_e[npr.W_1]*np.exp(-((i-n_e-2)/(1.*n_e/(2*np.pi)))**2/2./(self.handler.p_e[npr.W_SIGMA])**2)
        ...
        if self.params["gen"]["do_bistab_pert"]:
            ...
            nest.OneToOneConnect(ex_signal_init*sig_len, [ex_neurons[i] for i in sig_range], params = {'weight': 2., 'receptor_type': ex_receptors['AMPA']}, model= 'static_synapse')
        ...
        bump_t_start = self.params["gen"]["sig_start"]+self.params["gen"]["sig_len"]+1000. # 1s of buffer between start and avg
        if bump_t_end<=bump_t_start:
            bump_t_start_old = bump_t_start
            bump_t_start = self.params["gen"]["sig_start"]+self.params["gen"]["sig_len"]
            print "Run is not long enough (t_max: %.1f, bump_t_start: %.1f, bump_t_end: %.1f). Setting bump_t_start = %.1f" % (self.params["gen"]["tmax"],bump_t_start_old,bump_t_end,bump_t_start)
    ...
    def set_paramset(self,todo): # bump activity scanning
        if todo=="bump":
    ...
    def get_rates(self,t_start,t_stop):
        ...
        ex_spiketrains_base = signals.SpikeList(self.spikes["e"]["spike_list"],self.spikes["e"]["ids"],t_start=t_start, t_stop=t_stop)
        ex_base = ex_spiketrains_base.mean_rate()
        in_spiketrains_base = signals.SpikeList(self.spikes["i"]["spike_list"],self.spikes["i"]["ids"],t_start=t_start, t_stop=t_stop)
